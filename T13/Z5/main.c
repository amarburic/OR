...
int main()
{
    ...
    if((fajl = fopen("imenik.txt", "a+")) == NULL){
        printf("Fajl se ne moze otvoriti\n");
        exit(1);
    }
    while(fscanf(fajl, "%15s%20s %d\n",
    imenik[i].ime, imenik[i].prezime, &imenik[i].telefon) == 3)
        i++;
    while (!prekid) {
        ...
        case 1:
            imenik[i] = unos_osobe();
            fprintf(fajl, "%15s%20s %d\n",
            imenik[i].ime, imenik[i].prezime, imenik[i].telefon);
            i++;
            break;
        case 2:
            for (j = 0; j < i; j++) {
